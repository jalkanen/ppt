...
VOID WriteHeader( FRAME *frame, STRPTR basename, BPTR fh, EXTBASE *ExtBase )
{
...
VOID WriteFooter( FRAME *frame, BPTR fh, EXTBASE *ExtBase )
{
    fhprintf(fh,ExtBase,
                "\n};\n"
                "\n\n"
...
VOID WriteColormap( FRAME *frame, STRPTR basename, BOOL alpha, BPTR fh, EXTBASE *ExtBase )
{
    int i;
    ...
    for( i = 0; i < frame->disp->ncolors; i++ ) {
        if( alpha )  {
            fhprintf(fh,ExtBase,"%u,",frame->disp->colortable[i].a);
        }
           ...
           REG(a5) EXTBASE *ExtBase )
{
    ...
    WriteHeader( frame, basename, fh, ExtBase );

    if( format == CSF_LUT ) {
        WriteColormap( frame, basename, FALSE, fh, ExtBase );
    ...
    for( row = 0; row < frame->pix->height; row++ ) {
        ...
        if( format != CSF_LUT ) {
            ...
            for( col = 0; col < frame->pix->width*frame->pix->components; col++ ) {
                ...
                fhprintf( fh, ExtBase, "0x%02X,", cp[col] );
            }
        } else {
            PLANEPTR cp[8];
            int i;
            ...
            for( col = 0; col < frame->pix->width; col++ ) {
                if( col % 20 == 0 ) fhprintf(fh,ExtBase,"\n    ");
                fhprintf( fh, ExtBase, "0x%02X,", buf[col] );
            }

        }
    }

    WriteFooter( frame, fh, ExtBase );
